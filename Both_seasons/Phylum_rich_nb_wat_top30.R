...
pDADAwang1w<-subset_samples(DADAwang1, substrate_type=="water")
DADAwang1w<-subset_samples(pDADAwang1w, !cluster==2)
doi<-data.frame(sample_data(DADAwang1w))
...
newddw<-subset_samples(DADAwang1w, !(sample_root %in% c(with_NA1, with_NA2)))
...
plusP<-c("Ctenophora","Hyphochytriomycota")
...
setwd("/home/mpavila/eDNA/faststorage/Velux/CoastSequence/Autumn/Gua18s/both_seasons/results/regs/gllvm/wat")
...
formula = ~ season + habitat_rocks + habitat_eel + Salinity + Chlorophyll + season * (habitat_rocks + habitat_eel + Salinity + Chlorophyll), row.eff = ~(1 | cluster), family = "negative.binomial", method = "VA", control.start = list(starting.val ="zero"))
###!!###$$###!!###
...
#
fit_e2.1 <- gllvm(rich_asv, do, num.lv = 3,formula = ~ season + habitat_rocks + habitat_eel + Chlorophyll, row.eff = ~(1 | cluster), family = "negative.binomial", method = "VA", control.start = list(starting.val ="zero"))
###!!###$$###!!###

#
fit_e3 <- gllvm(rich_asv, do, num.lv = 3,formula = ~ season + habitat_rocks + habitat_eel + Chlorophyll + season * (habitat_rocks + habitat_eel + Chlorophyll), row.eff = ~(1 | cluster), family = "negative.binomial", method = "VA", control.start = list(starting.val ="zero"))
###!!###$$###!!###

#
fit_e4 <- gllvm(rich_asv, do, num.lv = 3,formula = ~ season + Salinity + Chlorophyll + season * (Salinity + Chlorophyll), row.eff = ~(1 | cluster), family = "negative.binomial", method = "VA", control.start = list(starting.val ="zero"))
###!!###$$###!!###

#
fit_e5 <- gllvm(rich_asv, do, num.lv = 3,formula = ~ season + habitat_rocks + habitat_eel + Salinity + Chlorophyll, row.eff = ~(1 | cluster), family = "negative.binomial", method = "VA", control.start = list(starting.val ="zero"))
###!!###$$###!!###

#
fit_e6 <- gllvm(rich_asv, do, num.lv = 3,formula = ~ habitat_rocks + habitat_eel + Salinity + Chlorophyll, row.eff = ~(1 | cluster), family = "negative.binomial", method = "VA", control.start = list(starting.val ="zero"))
###!!###$$###!!###
...
fit_e2.1
rescov.2.1 <- getResidualCov(fit_e2.1)
rescov.2.1$trace
rescov.2.1$var.q
1 - rescov.2.1$trace / rescov0$trace
...
pdf("phylum_wat_diagn_nb_rich_null.pdf")
plot(fit_0, mfrow = c(3,2),var.colors = 1) 
dev.off()

pdf("phylum_wat_diagn_nb_rich_full.pdf")
...
Colc <- cbPal[as.numeric(cut(jajac, breaks = 5))]
ordiplot(fit_0, symbols = T, s.colors = Colc, main = "chlorophyll-a", cex.spp=0.4, which.lvs = c(1,2))
legend("topleft", legend = c("0-0.3","0.3-0.8","0.8-2.2","2.2-5.5","5.5-13.4"), col=cbPal, pch=1, bty = "n")
...
pdf("ordi_phylum_nb_wat_rich_null3r_LVs13_ug.pdf")
par(mfrow = c(2,2), mar=c(2,2,2,2))
Col <- rbPal[as.numeric(cut(dp[,"legend"], breaks = 2))]
...
legend("topleft", legend = c("7-11","12-17","18-22","23-27","28-33"), col=sbPal2, pch=1, bty = "n")
Colc <- cbPal[as.numeric(cut(jajac, breaks = 5))]
ordiplot(fit_0, symbols = T, s.colors = Colc, main = "chlorophyll-a", cex.spp=0.4, which.lvs = c(1,3))
legend("topleft", legend = c("0-0.3","0.3-0.8","0.8-2.2","2.2-5.5","5.5-13.4"), col=cbPal, pch=1, bty = "n")
Colh <- hbPal[as.numeric(cut(dp[,"pchh"], breaks = 3))]
...
pdf("ordi_phylum_nb_wat_rich_null3r_LVs23_ug.pdf")
par(mfrow = c(2,2), mar=c(2,2,2,2))
Col <- rbPal[as.numeric(cut(dp[,"legend"], breaks = 2))]
...
legend("topleft", legend = c("7-11","12-17","18-22","23-27","28-33"), col=sbPal2, pch=1, bty = "n")
Colc <- cbPal[as.numeric(cut(jajac, breaks = 5))]
ordiplot(fit_0, symbols = T, s.colors = Colc, main = "chlorophyll-a", cex.spp=0.4, which.lvs = c(2,3))
legend("topleft", legend = c("0-0.3","0.3-0.8","0.8-2.2","2.2-5.5","5.5-13.4"), col=cbPal, pch=1, bty = "n")
Colh <- hbPal[as.numeric(cut(dp[,"pchh"], breaks = 3))]
...
ggsave("ordi_phylum_nb_wat_rich_full_old.pdf")
...
theme(axis.text.y = element_text(size = 5), axis.text.x = element_text(size=5, vjust=0.5), strip.text = element_text(size=5), legend.title=element_text(size=4), legend.text=element_text(size=4), axis.ticks.length=unit(.04, "cm"), legend.key.size = unit(0.4, "cm")) +
coord_cartesian(xlim = c(min(caca$LV1), max(caca$LV1)), ylim = c(min(caca$LV2),max(caca$LV2)))
ggsave("ordi_phylum_nb_wat_rich_full_old2.pdf")
...
ggsave("ordi_phylum_nb_wat_rich_null_old.pdf")
...
theme(axis.text.y = element_text(size = 5), axis.text.x = element_text(size=5, vjust=0.5), strip.text = element_text(size=5), legend.title=element_text(size=4), legend.text=element_text(size=4), axis.ticks.length=unit(.04, "cm"), legend.key.size = unit(0.4, "cm")) +
coord_cartesian(xlim = c(min(caca0$LV1), max(caca0$LV1)), ylim = c(min(caca0$LV2),max(caca0$LV2)))
ggsave("ordi_phylum_nb_wat_rich_null_old2.pdf")
...
ad0<-pad0[,c(ds)]
caca0<-cbind(ad0,do)
colnames(caca0)[1:2]<-c("LV3","LV1")
cent2 <- aggregate(cbind(LV3,LV1)~sshc,caca0,mean)
colnames(cent2)[2:3]<-c("cLV3","cLV1")
segs <- merge(caca0, setNames(cent2, c("sshc","cLV3","cLV1")), by = "sshc", sort = FALSE)
segs$sh<-paste(segs$season, segs$habitat)
...
ggplot(data=segs, aes(x=LV3, y=LV1)) +
geom_segment(aes(xend = cLV3, yend = cLV1, colour = sh), alpha=0.3, size=0.05) + 
geom_point(aes(cLV3, cLV1, colour = sh), shape=1, size = 2.5, alpha=0.8) +
geom_text(aes(cLV3, cLV1, label=cluster), size=1.7, color="gray20") +
geom_hline(yintercept=0, linetype="dotted") +
geom_vline(xintercept=0, linetype="dotted") +
theme_bw() +
scale_colour_manual(values = c("red3","darkgreen","blue3","indianred1","lightgreen","lightskyblue1")) +
xlab(paste("LV3","-",expvar[1],"%",sep=" ")) + ylab(paste("LV1","-",expvar[2],"%",sep=" ")) +
theme(axis.text.y = element_text(size = 5), axis.text.x = element_text(size=5, vjust=0.5), strip.text = element_text(size=5), legend.title=element_text(size=4), legend.text=element_text(size=4), axis.ticks.length=unit(.04, "cm"), legend.key.size = unit(0.4, "cm")) +
coord_cartesian(xlim = c(min(caca0$LV1), max(caca0$LV1)), ylim = c(min(caca0$LV2),max(caca0$LV2)))
ggsave("ordi_phylum_nb_wat_rich_null.pdf")


#ggsave("wat_ordi_phylum_neg_b_gllvm_subs_habs_random_full_0_va_top30.pdf")

pdf("wat_phylum_env_effect_nb_rich_va_top30.pdf")
coefplot(fit_e, cex.ylab = 0.5, mar = c(4, 5.5, 2, 1), mfrow=c(1,2))
dev.off()

cr <- getResidualCor(fit_e)

pdf("wat_phylum_nb_rich_corr_va_top30.pdf")
...
pdf("wat_phylum_nb_rich_corr_0_va_top30.pdf")
...
pdf("wat_phylum_nb_rich_envicorr_va_top30.pdf")
...
#Sorted predictors
spred<-c("season","habitat_rocks","habitat_eel","Salinity","Chlorophyll",
"season*habitat_rocks","season*habitat_eel","season*Salinity","season*Chlorophyll")
...
ggsave("wat_summary_heatmap_env_eff_phylum_nb_rich_va_top30.pdf")
...
scale_y_discrete(expand=c(0,0)) +
theme(axis.text.x = element_text(angle = 60, hjust = 1), axis.text=element_text(size=8))
ggsave("wat_Meta_summary_heatmap_env_eff_phylum_nb_rich_va_top30.pdf")
...
scale_y_discrete(expand=c(0,0)) +
theme(axis.text.x = element_text(angle = 60, hjust = 1), axis.text=element_text(size=8))
ggsave("wat_Non_meta_summary_heatmap_env_eff_phylum_nb_rich_va_top30.pdf")
