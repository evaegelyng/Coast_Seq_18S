...
#Write info
beta_br[beta_br$Quantile==quant[i],"Total_reads"]<-sum(sample_sums(p_ncbi))
beta_br[beta_br$Quantile==quant[i],"Total_ASVs"]<-ntaxa(p_ncbi)
beta_br[beta_br$Quantile==quant[i],"Field_replicates"]<-nsamples(p_ncbi)
beta_br[beta_br$Quantile==quant[i],"Depth"]<-ifelse(length(levels(as.factor(sample_sums(p_ncbi))))==1, levels(as.factor(sample_sums(p_ncbi))), mean(sample_sums(p_ncbi)))
...
ggsave(filename = paste("quantile",quant[i],"ord_bc_root_ncbi.pdf", sep="_"), plot = plot)
...
write.table(beta_br, "summary_depth_cutoff_ncbi.txt", sep="\t", quote=FALSE)
